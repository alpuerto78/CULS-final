    ...
    class NetworkClient
    {
        ...
        public static void ConnectClient()
        {
            ...
            try
            {
                ...
                _ipAd = exec_ini["IPAddress"]["IPAd"];
            }
            catch (Exception ex)
            {
             //   MessageBox.Show(ex.Message, _title, MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            Handler_Connection_Flag h1 = new Handler_Connection_Flag();

           clientSocket = new TcpClient();
           
                if (h1.Handler_conn_flag !=true )
                {
                    try
                    {
                        ...
                        ctThread.Start();
                        h1.Handler_conn_flag =true;
                }
                    catch (Exception er)
                    {
                 //   MessageBox.Show(er.Message, _title);
        ...
        static bool SocketConnected(TcpClient s) //check whether client is connected server
        {
            ...
            {
                MessageBox.Show(ex.Message, "Dito may error");
